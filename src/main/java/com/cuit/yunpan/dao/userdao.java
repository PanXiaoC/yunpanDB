...
public interface userdao {
    ...
    //查询所有的共享文件(已测试)
    @Select("select *from myfiles where is_delete=#{is_delete}")
    public List<myfiles> findshare_myfiles(Integer is_delete);
    //查询myfiles中某个user的所有文件总和；
    @Select("SELECT SUM(file_size) FROM myfiles WHERE user_id=#{user_id}")
    public long sumOneFilesByUserid(userinfo user);
    //设置文件为共享文件，is_delete=1
    @Update("update myfiles set is_delete=1 where id=#{id}")
    public boolean setIs_deleteAs1(Integer id);
}
